    ...
    <Field className="flex grow items-baseline justify-center gap-6">
      ...
      <Combobox value={selected} onChange={onChange} onClose={() => setQuery('')}>
        <div className="relative">
          <ComboboxInput
            className={clsx(
              'w-full rounded-lg border-none bg-black/5 dark:bg-white/5 py-1.5 pr-8 pl-3 text-sm/6 text-black dark:text-white',
              'focus:not-data-focus:outline-none data-focus:outline-2 data-focus:-outline-offset-2 data-focus:outline-gray/25 dark:data-focus:outline-white/25'
            )}
            displayValue={(item: dropBoxModel) => item?.name}
            onChange={(event) => setQuery(event.target.value)}
          />
          <ComboboxButton className="group absolute inset-y-0 right-0 px-2.5">
            <ChevronDownIcon className="size-4 fill-black/60 group-data-hover:fill-black dark:fill-white/60 dark:group-data-hover:fill-white" />
          </ComboboxButton>
        </div>
        ...
        <ComboboxOptions
          ...
          transition
          className={clsx(
            'w-(--input-width) rounded-xl border border-black/5 bg-black/5 p-1 dark:border-white/5 dark:bg-white/5 p-1 [--anchor-gap:--spacing(1)] empty:invisible',
        ...
        >
          {filteredItems.map((item) => (
            <ComboboxOption
              key={item.id}
              value={item}
              className="group flex cursor-default items-center gap-2 rounded-lg px-3 py-1.5 select-none data-focus:bg-black/10 dark:data-focus:bg-white/10"
            >
